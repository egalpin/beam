...
import com.dialpad.pubsubToElasticsearch.PubsubToEsOptions;
...
public class ElasticsearchIO {
    ...
    public static Write write() {
        return new AutoValue_ElasticsearchIO_Write.Builder()
                ...
                .setIgnoreVersionConflicts(false)
                .build();
    }
    ...
    static void checkForErrors(
            ...
            boolean ignoreVersionConflicts)
            throws IOException {
        ...
        if (errors) {
            ...
            for (JsonNode item : items) {
                ...
                if (type.equals(VERSION_CONFLICT_ERROR) && ignoreVersionConflicts) {
                    continue;
                }
                ...
                errorMessages.append(
                        String.format("%nDocument id %s: %s (%s)", docId, reason, type));
    ...
    public abstract static class ConnectionConfiguration implements Serializable {
        ...
        public static ConnectionConfiguration create(
                String[] addresses, String index, String type) {
        ...
        /**
         * If Elasticsearch uses SSL/TLS with mutual authentication (via shield), provide the
         * keystore containing the client key.
         *
        ...
        /**
         * If Elasticsearch uses SSL/TLS with mutual authentication (via shield), provide the
         * password to open the client keystore.
         *
        ...
        /**
         * If set, overwrites the default max retry timeout (30000ms) in the Elastic {@link
         * RestClient} and the default socket timeout (30000ms) in the {@link RequestConfig} of the
        ...
        /**
         * If set, overwrites the default connect timeout (1000ms) in the {@link RequestConfig} of
         * the Elastic {@link RestClient}.
         *
        ...
        private void populateDisplayData(DisplayData.Builder builder) {
            ...
            builder.addIfNotNull(
                    DisplayData.item("socketAndRetryTimeout", getSocketAndRetryTimeout()));
            builder.addIfNotNull(DisplayData.item("connectTimeout", getConnectTimeout()));
            builder.addIfNotNull(
                    DisplayData.item("trustSelfSignedCerts", isTrustSelfSignedCerts()));
        }
    ...
    public abstract static class Read extends PTransform<PBegin, PCollection<String>> {
        ...
        abstract boolean getForceNoScroll();

        abstract long getBatchSize();

        @Nullable
        abstract Integer getAssumedBackendVersion();
        ...
        abstract static class Builder {
            ...
            abstract Builder setForceNoScroll(boolean forceNoScroll);

            abstract Builder setBatchSize(long batchSize);

            abstract Builder setAssumedBackendVersion(Integer assumedBackendVersion);

            abstract Read build();
        }

        public Read withAssumedBackendVersion(int backendVersion) {
            checkArgument(
                    backendVersion == 2
                            || backendVersion == 5
                            || backendVersion == 6
                            || backendVersion == 7,
                    "Backend version may only be one of 2, 5, or 6");
            return builder().setAssumedBackendVersion(backendVersion).build();
        }
        ...
        public Read withConnectionConfiguration(ConnectionConfiguration connectionConfiguration) {
            checkArgument(
                    connectionConfiguration != null, "connectionConfiguration can not be null");
        ...
        /**
         * Include metadata in result json documents. Document source will be under json node
         * _source.
         *
         * @return a {@link PTransform} reading data from Elasticsearch.
         */
        ...
        public Read withForceNoScroll() {
            return builder().setForceNoScroll(true).build();
        }
         ...
         * scroll API</a> Default is 100. Maximum is 10 000. If documents are small, increasing
         * batch size might improve read performance. If documents are big, you might need to
         * decrease batchSize
         *
    ...
    public static class BoundedElasticsearchSource extends BoundedSource<String> {
        ...
        public List<? extends BoundedSource<String>> split(
            ...
            if (spec.getAssumedBackendVersion() == null) {
                this.backendVersion = getBackendVersion(connectionConfiguration);
            } else {
                this.backendVersion = spec.getAssumedBackendVersion();
            }
            List<BoundedElasticsearchSource> sources = new ArrayList<>();
            if (backendVersion == 2) {
                // 1. We split per shard :
                // unfortunately, Elasticsearch 2.x doesn't provide a way to do parallel reads on a
                // single
                ...
                JsonNode statsJson =
                        BoundedElasticsearchSource.getStats(connectionConfiguration, true);
                JsonNode shardsJson =
                        statsJson
                                .path("indices")
                ...
                while (shards.hasNext()) {
                    ...
                    sources.add(
                            new BoundedElasticsearchSource(
                                    spec, shardId, null, null, backendVersion));
                }
                checkArgument(!sources.isEmpty(), "No shard found");
            } else if (backendVersion == 5 || backendVersion == 6 || backendVersion == 7) {
                if (spec.getForceNoScroll()) {
                    sources.add(
                            new BoundedElasticsearchSource(spec, null, null, null, backendVersion));
                } else {
                    long indexSize =
                    ...
                    for (int i = 0; i < nbBundles; i++) {
                        sources.add(
                                new BoundedElasticsearchSource(
                                        spec, null, nbBundles, i, backendVersion));
                    }
                }
            }
            return sources;
        ...
        static long estimateIndexSize(ConnectionConfiguration connectionConfiguration)
            ...
            JsonNode indexStats =
                    statsJson
                            ...
                            .path(connectionConfiguration.getIndex())
                            .path("primaries");
            if (indexStats.isMissingNode()) {
                indexStats = statsJson.path("_all").path("primaries");
            }
    ...
    private static class BoundedElasticsearchReader extends BoundedSource.BoundedReader<String> {
        ...
        public boolean start() throws IOException {
            ...
            if (!source.spec.getForceNoScroll()) {
        ...
        public boolean advance() throws IOException {
            if (source.spec.getForceNoScroll()) {
                ...
                HttpEntity scrollEntity =
                        new NStringEntity(requestBody, ContentType.APPLICATION_JSON);
     ...
     * will be attempted until the maxAttempts or maxDuration is exceeded, whichever comes first,
     * for 429 TOO_MANY_REQUESTS error.
     */
    @AutoValue
    public abstract static class RetryConfiguration implements Serializable {
        ...
        abstract static class Builder {
            ...
            abstract ElasticsearchIO.RetryConfiguration.Builder setMaxDuration(
        ...
        /**
         * An interface used to control if we retry the Elasticsearch call when a {@link Response}
         * is obtained. If {@link RetryPredicate#test(Object)} returns true, {@link Write} tries to
         * resend the requests to the Elasticsearch server if the {@link RetryConfiguration} permits
         * it.
         */
        @FunctionalInterface
        ...
        static class DefaultRetryPredicate implements RetryPredicate {
            ...
            private static boolean errorCodePresent(HttpEntity responseEntity, int errorCode) {
                try {
                    ...
                    LOG.warn(
                            "Could not extract error codes from responseEntity {}", responseEntity);
                }
                return false;
            }
    ...
    public abstract static class Write extends PTransform<PCollection<String>, PDone> {
         ...
         * This is used for using explicit document ids, and for dynamic routing (index/Type) on a
         * document basis. A null response will result in default behaviour and an exception will be
        ...
        @Nullable
        abstract FieldValueExtractFn getRoutingFn();

        @Nullable
        abstract FieldValueExtractFn getTypeFn();

        @Nullable
        abstract FieldValueExtractFn getDocVersionFn();

        @Nullable
        abstract String getDocVersionType();
        ...
        abstract boolean getIgnoreVersionConflicts();

        @Nullable
        abstract String getUpsertScript();
        ...
        abstract static class Builder {
            ...
            abstract Builder setRoutingFn(FieldValueExtractFn routingFunction);

            abstract Builder setIndexFn(FieldValueExtractFn indexFn);

            abstract Builder setTypeFn(FieldValueExtractFn typeFn);

            abstract Builder setDocVersionFn(FieldValueExtractFn docVersionFn);

            abstract Builder setUsePartialUpdate(boolean usePartialUpdate);

            abstract Builder setIgnoreVersionConflicts(boolean ignore);
        ...
        public String DocToBulk(String document, int backendVersion) throws IOException {
            return WriteFn.DocToBulk(this, document, backendVersion);
        }
         ...
         * (https://www.elastic.co/guide/en/elasticsearch/reference/2.4/docs-bulk.html). Default is
         ...
         * execution engine, size of bundles may vary, this sets the maximum size. Change this if
         * you need to have smaller ElasticSearch bulks.
         *
        ...
        public Write withMaxBatchSizeBytes(long batchSizeBytes) {
            checkArgument(
                    batchSizeBytes > 0, "batchSizeBytes must be > 0, but was %s", batchSizeBytes);
            return builder().setMaxBatchSizeBytes(batchSizeBytes).build();
        }

        /**
         * Provide a function to extract the doc version from the document. This version number will
         * be used as the document version in Elasticsearch. Should the function throw an Exception
         * then the batch will fail and the exception propagated. Incompatible with update
         * operations and should only be used with withUsePartialUpdate(false)
         *
        ...
        public Write withIgnoreVersionConflicts(boolean ignore) {
            return builder().setIgnoreVersionConflicts(ignore).build();
        }

        /**
         * Provide a function to extract the id from the document. This id will be used as the
         * document id in Elasticsearch. Should the function throw an Exception then the batch will
         * fail and the exception propagated.
         *
        ...
        /**
         * Provide a function to extract the target type from the document allowing for dynamic
         * document routing. Should the function throw an Exception then the batch will fail and the
         * exception propagated. Users are encouraged to consider carefully if multipe types are a
         * sensible model <a
        ...
        /**
         * Provide an instruction to control whether partial updates or inserts (default) are issued
         * to Elasticsearch.
         *
        ...
        /**
         * Provide an instruction to control whether script updates are issued to Elasticsearch, and
         * prodive the script itself.
         *
         * @param source set to the value of the script source, painless lang
         * @return the {@link Write} with the script update control set
         */
        ...
        /**
         * Provides configuration to retry a failed batch call to Elasticsearch. A batch is
         * considered as failed if the underlying {@link RestClient} surfaces 429 HTTP status code
         * as error for one or more of the items in the {@link Response}. Users should consider that
         * retrying might compound the underlying problem which caused the initial failure. Users
         * should also be aware that once retrying is exhausted the error is surfaced to the runner
         * which <em>may</em> then opt to retry the current bundle in entirety or abort if the max
         * number of retries of the runner is completed. Retrying uses an exponential backoff
         * algorithm, with minimum backoff of 5 seconds and then surfacing the error once the
         * maximum number of retries or maximum configuration duration is exceeded.
         *
         * <p>Example use:
         *
         * <pre>{@code
        ...
        public Write withAssumedBackendVersion(int backendVersion) {
            checkArgument(
                    VALID_CLUSTER_VERSIONS.contains(backendVersion),
                    "Backend version may only be one of 2, 5, 6, or 7");
            return builder().setAssumedBackendVersion(backendVersion).build();
        }

        @Override
        public PDone expand(PCollection<String> input) {
            PubsubToEsOptions options = (PubsubToEsOptions) input.getPipeline().getOptions();
                            ...
                            ParDo.of(
                                    new DoFn<Integer, Void>() {
                                        ...
                                        public void processElement(ProcessContext c) {
                                            // do nothing. Just keeps track of inputs from previous
                                            // step in pipeline so as to provide
                                            // data on bottlenecks.
                                        }
                                    }));
        ...
        @VisibleForTesting
        static class WriteFn extends DoFn<String, Integer> {
            ...
            @VisibleForTesting
            static final String RETRY_ATTEMPT_LOG =
            ...
            public void setup() throws IOException {
                ...
                if (spec.getAssumedBackendVersion() == null) {
                    ...
                    backendVersion = spec.getAssumedBackendVersion();
                }
                ...
                if (spec.getRetryConfiguration() != null) {
                    retryBackoff =
                            FluentBackoff.DEFAULT
                                    .withInitialBackoff(RETRY_INITIAL_BACKOFF)
                                    .withMaxRetries(
                                            spec.getRetryConfiguration().getMaxAttempts() - 1)
                                    .withMaxCumulativeBackoff(
                                            spec.getRetryConfiguration().getMaxDuration());
                }
            }
            ...
            /**
             * Extracts the components that comprise the document address from the document using
             * the {@link FieldValueExtractFn} configured. This allows any or all of the index, type
             * and document id to be controlled on a per document basis. If none are provided then
             * an empty default of {@code {}} is returned. Sanitization of the index is performed,
             * automatically lower-casing the value as required by Elasticsearch.
             *
            ...
            private static String getDocumentMetadata(
                    Write spec, String document, int backendVersion) throws IOException {
                    ...
                    DocumentMetadata metadata =
                            new DocumentMetadata(
                                    spec.getIndexFn() != null
                                            ? lowerCaseOrNull(
                                                    spec.getIndexFn().apply(parsedDocument))
                                            : null,
                                    spec.getTypeFn() != null
                                            ? lowerCaseOrNull(
                                    ...
                                    spec.getRoutingFn() != null
                                            ? spec.getRoutingFn().apply(parsedDocument)
                                            : null,
                                    backendVersion,
                                    spec.getDocVersionFn() != null
                                            ? spec.getDocVersionFn().apply(parsedDocument)
                                            : null,
            ...
            static String DocToBulk(Write spec, String document, int backendVersion)
                    throws IOException {
                String documentMetadata = getDocumentMetadata(spec, document, backendVersion);
            ...
            public void processElement(ProcessContext context) throws Exception {
                ...
                batch.add(DocToBulk(spec, document, backendVersion));
                ...
                if (batch.size() >= spec.getMaxBatchSize()
                    ...
                    int numFlushed = flushBatch();

                    // Just output the same number of arbitrary elements as there were documents
                    // successfully indexed into ES. This will help track when ES ingestion is the
                    // bottleneck.
                    IntStream.range(0, numFlushed).forEach(context::output);
                }
            }
            ...
            private int flushBatch() throws IOException, InterruptedException {
                if (batch.isEmpty()) {
                    return 0;
                }
                ...
                if (spec.getRetryConfiguration() != null
                    ...
                    responseEntity =
                            handleRetry("POST", endPoint, Collections.emptyMap(), requestBody);
                }
                checkForErrors(
                        responseEntity,
                        backendVersion,
                        spec.getUsePartialUpdate(),
                        spec.getIgnoreVersionConflicts());

                return prevBatchSize;
            }
    ...
    static int getBackendVersion(ConnectionConfiguration connectionConfiguration) {
        try (RestClient restClient = connectionConfiguration.createClient()) {
            ...
            int backendVersion =
                    Integer.parseInt(
                            jsonNode.path("version").path("number").asText().substring(0, 1));
            checkArgument(
                    (VALID_CLUSTER_VERSIONS.contains(backendVersion)),
